...
**Decontamination.** Word 8-gram overlap between a dataset's prompts and replies and any evaluation source: row lists, JSONL paths, or platform dataset ids. Short prompts fall back to exact match. Returns the clean rows and the first offenders. `fields=` picks what is compared (default `("prompt", "final_text")`) and `report["by_field"]` counts how many rows each one flagged — check it before believing a high rate, because two runs of one simulator share both the writer's stock phrasing and the policy's own replies, and that reads as near-100% contamination with no evaluation prompt reused. Raise `n` or narrow `fields` when `by_field` shows that is what happened.
