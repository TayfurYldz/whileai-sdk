...
zps.decontaminate(train_rows, against=[eval_rows])  # 8-gram overlap, training prompt to eval prompt
...
**Decontamination.** Word 8-gram overlap between a dataset's prompts and any evaluation source: row lists, JSONL paths, or platform dataset ids. Short prompts fall back to exact match. Returns the clean rows and the first offenders. `fields=` names the training fields compared, and defaults to `("prompt",)` — the field standard is overlap from the training prompt to the eval prompts (rlhf-book ch. 16, the Tülu 3 8-gram decontamination). `fields=("prompt", "final_text")` also compares the row's own reply, which answers a different and stricter question ("did this policy reproduce eval answers?"); two runs of the same policy share reply phrasing by construction, so on same-generator data it flags nearly everything. `by_field` in the report says which field drove the matches, and a contamination rate at or above 0.9 comes with a warning that shared templates, not leakage, are the usual cause.
