...
def test_train_modal_imports_and_its_flags_match_the_readme():
    ...
    assert_readme_matches_entrypoints(README, {"recipes/04-train/dpo/train_modal.py": mod.main})
...
def test_readme_has_no_placeholders_and_names_only_real_things():
    ...
    assert "recipes/04-train/grpo/prompts.jsonl" in text and (GRPO / "prompts.jsonl").exists()
    _, p = _modules()
