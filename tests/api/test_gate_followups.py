...
def test_push_file_gates_and_uploads_stamped_rows(monkeypatch, tmp_path):
    ...
    out = wai.push_file(str(good), api_key="k")
    ...
    with pytest.raises(PublishGateError, match="no_mixed_groups"):
        wai.push_file(str(bad), api_key="k")
    raw = wai.push_file(str(bad), api_key="k", gate=False)
    assert "gate" not in raw and uploads[-1] == bad.read_bytes()


def test_calibration_round_trips_typed():
    rows = _rows({"a": [1, 0, 1, 1]})
    wai.calibrate(rows, policy="Look up first.", model="qwen")
    row = wai.to_row(*wai.from_row(schema.stamp(dict(rows[0]))))
    assert row["calibration"]["pass_rate"] == 0.75 and row["calibration"]["n"] == 4
    cal = wai.calibration_of(row)
    ...
    _task, rollout, _j, _m = wai.from_row(row)
    assert isinstance(rollout.extra["calibration"], schema.Calibration)
    assert "calibration" not in (rollout.extra.get("passthrough") or {})

    assert wai.calibration_of({"prompt": "x"}) is None
    assert wai.validate(schema.stamp({"prompt": "x", "calibration": {"n": 0, "pass_rate": 2}})) == [
        "calibration_invalid"
    ]
    assert (
        wai.validate(schema.stamp({"prompt": "x", "calibration": {"n": 3, "pass_rate": 0.5}})) == []
    )


def test_training_rows_carry_a_loss_mask_on_assistant_turns_only():
    out = wai.training_rows(_rows({"a": [1]}), system_prompt="Be careful.", tools=[{"x": 1}])
    entry = out[0]
    roles = [m["role"] for m in entry["messages"]]
    assert roles == ["system", "user", "assistant", "tool", "assistant"]
    assert entry["loss_mask"] == [0, 0, 1, 0, 1]
    assert wai.validate(entry, "training") == []
    broken = dict(entry, loss_mask=[1, 0])
    assert wai.validate(broken, "training") == ["loss_mask_invalid"]
    schema_doc = wai.schema.load_json_schema()
