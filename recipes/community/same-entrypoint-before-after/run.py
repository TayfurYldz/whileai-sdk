...
def reference_agent(message: str) -> dict:
    ...
    return {"steps": [{"tool": "lookup_invoice", "args": {"invoice_id": inv.group(0)},
                       "result": {"invoice_id": inv.group(0), "amount_usd": 109.0,
                                  "status": "open"}}],
...
def build_tasks(budget: int) -> list[dict]:
    ...
    data = wai.simulate(reference_agent, tools=TOOLS, system_prompt=POLICY,
                        budget=budget, simulator=False, seed=SEED, reproducible=True)
    rows = data.rows()
    ...
    print(f"corpus {len(rows)} rows | held_out {len(held)} rows / {len(hk)} tasks "
...
def run_arm(tasks: list[dict], model: str, tag: str, repeats: int, seed: int) -> list[dict]:
    agent = wai.local_model(ENDPOINT, model, tools=TOOLS, system=POLICY,
                            api_key=resolve_api_key(), temperature=0.8, timeout=120)
    data = wai.simulate(agent, tools=TOOLS, system_prompt=POLICY, tasks=tasks,
                        rollouts_per_request=repeats, grader=grade,
    ...
    pa = wai.pass_at(rows)
    print(f"[{tag}] model={model} rows={len(rows)} "
...
def _markers(label: str, rows: list[dict]) -> dict:
    ...
    for name, m in wai.marker_summary(rows).items():
        ...
        table[name] = {"mean": m.get("mean"), "ci95": m.get("ci95"),
                       "n_rows": m.get("n_rows"), "n_tasks": m.get("n_tasks")}
        print(f"  {name:26s} {m.get('mean')}  ci95={m.get('ci95')}  "
...
def report() -> None:
    ...
    for tag in ("base_1", "trained"):
        ...
        n_open = sum(1 for r in raw if "<think>" in (r.get("final_text") or "")
                     and "</think>" not in (r.get("final_text") or ""))
        kept = wai.pass_at(regrade(json.loads((OUT / f"{tag}.json").read_text()),
                                   keep_think=True)).pass_at_1
        stripped = wai.pass_at(regrade(json.loads((OUT / f"{tag}.json").read_text()))).pass_at_1
        sens[tag] = {"rows": len(raw), "rows_with_think": n_think, "unclosed_think": n_open,
                     "pass_at_1_keep_think": kept, "pass_at_1_stripped": stripped}
        print(f"  {tag:8s} <think> in {n_think}/{len(raw)} rows ({n_open} unclosed) | "
              f"pass@1 raw {kept:.3f} -> stripped {stripped:.3f}")

    results = {
        "whileai": __import__("whileai").__version__,
        "endpoint": ENDPOINT, "base_model": BASE_MODEL, "trained_model": TRAINED_MODEL,
        "adapter_run": "run_327b614f3682cae5",
        "pinned_tasks": len({wai.task_key(r) for r in b1}),
        "run_std": run_std, "eval_variance": ev,
        "pass_at_1": {"base_1": wai.pass_at(b1).pass_at_1,
        ...
        "delta": d, "null_control": n, "think_sensitivity": sens,
    }
...
def dry_run(limit: int) -> None:
    ...
    data = wai.simulate(reference_agent, tools=TOOLS, system_prompt=POLICY,
                        budget=max(4, limit * 2), simulator=False, seed=SEED,
                        reproducible=True, grader=grade)
    rows = data.rows()
...
def main() -> int:
    ...
    if args.step in ("all", "eval"):
        for tag, model, seed in [("base_1", BASE_MODEL, 101),
